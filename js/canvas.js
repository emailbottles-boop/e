...
function handleCanvasMouseDown(e) {
    ...
    const worldX = (canvasX - canvas.width / 2) / zoom + (canvas.width / 2 - panX);
    const worldY = (canvasY - canvas.height / 2) / zoom + (canvas.height / 2 - panY);
    ...
    if (drawMode !== DRAW_MODES.NONE && drawMode !== 'none' && isDM) {
        isDrawing = true;
        lastX = worldX;
        lastY = worldY;
        
        if (drawMode === DRAW_MODES.PEN || drawMode === 'pen') {
...
function handleCanvasMouseMove(e) {
    ...
    if (drawMode === DRAW_MODES.PEN || drawMode === 'pen' || drawMode === DRAW_MODES.ERASER || drawMode === 'eraser') {
        drawOnMap(lastX, lastY, worldX, worldY);
        lastX = worldX;
        lastY = worldY;
    }
    
    if (fogMode === FOG_MODES.REVEAL || fogMode === FOG_MODES.HIDE || fogMode === 'reveal' || fogMode === 'hide') {
        drawFog(worldX, worldY);
    }
}
...
function handleCanvasMouseUp(e) {
    ...
    if (drawMode !== DRAW_MODES.NONE && drawMode !== 'none' && isDM) {
        saveDrawHistory();
        if (typeof broadcastGridUpdate === 'function') {
            broadcastGridUpdate();
        }
    }
}
...
function importMapToGrid() {
    if (!mapCanvas || !mapCtx) {
        alert('❌ Map canvas not initialized!\n\nPlease draw something on the Map Builder first.');
        return;
    }
    
    // Check if there's actually content on the map canvas
    const imageData = mapCtx.getImageData(0, 0, mapCanvas.width, mapCanvas.height);
    const hasContent = imageData.data.some(channel => channel !== 0);
    
    if (!hasContent) {
        alert('❌ Map canvas is empty!\n\nDraw something on the Map Builder canvas first.');
        return;
    }
    
    const key = `${currentCellX},${currentCellY}`;
    
    console.log('Importing map to grid cell:', key);
    
    // Create or get the grid cell canvas
    if (!gridCells[key]) {
        gridCells[key] = document.createElement('canvas');
        gridCells[key].width = CELL_SIZE;
        gridCells[key].height = CELL_SIZE;
    ...
    cellCtx.drawImage(
        mapCanvas, 
        0, 0, mapCanvas.width, mapCanvas.height,  // Source
        0, 0, CELL_SIZE, CELL_SIZE                // Destination
    );
    
    console.log('✅ Map imported successfully to cell', key);
    ...
    // Broadcast to players if DM
    if (isDM && typeof broadcastGridUpdate === 'function') {
        broadcastGridUpdate();
    }
    
    // Save session
    if (isDM && typeof saveSessionData === 'function') {
        saveSessionData();
    }
    
    // Show success message
    const cellDisplay = `[${currentCellX + 1}, ${currentCellY + 1}]`;
    alert(`✅ Map Imported!\n\nYour drawing has been imported to grid cell ${cellDisplay}.\n\nCheck the main canvas!`);
}
